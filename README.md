...
从四声部众赞歌片段中，逐个声部听辨巴赫的原作。正式游戏由服务端抽题和判分，浏览器只加载当前这一局抽中的题目资源。
...
题库保留 45 道题：30 道四声部众赞歌、10 道赋格、5 道三声部 Sinfonia。正式游戏默认每局仅抽取 3 道众赞歌；赋格与创意曲保留在题库中，本次不参与抽题。每个声部固定提供 3 个选项。管理后台可在分类库存满足配额时调整每局题数、分类配额和评分权重。

默认评分为：完全猜中题目的数量占 25%，所有声部选项的正确比例占 75%。规则修改后由服务端递增规则版本；进行中的旧会话仍使用创建时保存的答案和规则快照。

题库元数据包含声部顺序、显示名称、谱号、调号、拍号、小节范围、来源许可和各声部最长休止检查结果。正式抽取的题目均为四声部众赞歌；正确组合不会新增平行五度或八度，干扰组合允许有限的声部进行或和声变化。
...
3. 动态核对 45 道保留题目及其音频、MusicXML 和预生成 SVG 资源。
