...
3. **和声变化有效性**：每条 `harmony` 候选必须在指定稳定拍改变四声部纵向音高类集合，避免只替换重复音或同一和弦内的八度位置。
4. **稳定拍不协和**：每两拍检查一次共同持续至少半拍的稳定拍不协和；不得超过原作的基线例外。
5. **平行纯音程**：在全部音符边界，不允许新增同向纯五度或纯八度；原作中已有实例记录为基线例外。
